...
bool MyUPnP::InvokeCommand(const CString& name, const CString& args, CString& strResponse)
{
	if(!isComplete())return false;
	CString post, host, addr;
	int port = 0;
...
MyUPnP::UPNPNAT_RETURN MyUPnP::AddNATPortMapping(UPNPNAT_MAPPING *mapping, bool tryRandom)
{
	...
	for (int retries = 255; retries; retries--) {
		CString Desc;
		Desc.Format(_T("Yuanjie (%s) [%s: %u]"), mapping->description, ProtoStr, mapping->externalPort);
...
bool	MyUPnP::GetSpecificPortMappingEntry(LPCTSTR lpszRemoteHost,					//[in]
...
bool	MyUPnP::GetGenericPortMappingEntry(USHORT usIndex,					//[in]
