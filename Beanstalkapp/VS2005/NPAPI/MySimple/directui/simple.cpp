...
// 	gpnpf->setvalue(instance, NPPVpluginWindowBool, 0);	// 强制绘制动作
// 	gpnpf->setvalue(instance, NPPVpluginTransparentBool, (void *)1);
...
NPError NPP_MyHandleEvent(NPP instance, void* event)
{
	...
	if(WM_PAINT == pEvent->event)
	{
		...
		ATLTRACE("[NPP_MyHandleEvent] WM_PAINT:{%d,%d,%d,%d}\n", rect.left, rect.top, rect.right, rect.bottom);
		//gpnpf->getvalue(instance, NPNVnetscapeWindow, (void *)&hWnd);
		//hDC = ::GetDC(hWnd);
