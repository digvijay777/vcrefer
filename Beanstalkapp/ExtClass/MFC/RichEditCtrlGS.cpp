...
BOOL CRichEditCtrlGS::AppenString(TCHAR *pString, CHARFORMAT *pCf)
{
	SetSel(-1, -1);
	ReplaceSel(pString);
...
{ 
	...
	es.dwCookie = (DWORD_PTR)&CookieRTF; // and the informations
...
{ 
	...
	CookieRTF.lSize = (long)fileStatus.m_size;
...
DWORD CALLBACK CRichEditCtrlGS::StreamInCString(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{
...
DWORD CALLBACK CRichEditCtrlGS::StreamInCByteArray(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{ 
	...
	*pcb = 0;            // Till now we have nothing read so far
	...
	{ 
		lMaxSize = (LONG)pCookie->pInText->GetSize();
	}
	lReadSize = lMaxSize;
	if( (pCookie->lSize - pCookie->lStart) < lReadSize )
	{ 
		lReadSize = (LONG)(pCookie->lSize - pCookie->lStart);
	}
	if( lReadSize <= 0 )
	{ 
		return (0); // Nothing to do
...
DWORD CALLBACK CRichEditCtrlGS::StreamInCFile(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{ 
...
DWORD CALLBACK CRichEditCtrlGS::StreamOutCString(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG* pcb)
{ 
 	SCookieString *pCookie = (SCookieString *)dwCookie;
...
DWORD CALLBACK CRichEditCtrlGS::StreamOutCByteArray(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG* pcb)
{ 
	SCookieByteArray *pCookie = (SCookieByteArray *)dwCookie;
	long lWriteSize = cb; // We can only Write such much
	*pcb = 0;             // Till now we have nothing written so far
...
DWORD CALLBACK CRichEditCtrlGS::StreamOutCFile(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{
	SCookieFile *pCookie = (SCookieFile *)dwCookie;
	*pcb = 0;             // Till now we have nothing written so far
	if( pCookie->lStart < 0 )
...
void CRichEditCtrlGS::OnEnLink(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	{
		SetSel(pEnLink->chrg.cpMin,pEnLink->chrg.cpMax);
	 	CString		strText = GetSelText();
	 	if( strText.GetLength() > 0 )
			ShellExecute( NULL, _T("open"), _T("iexplore.exe"), strText.GetBuffer(), NULL, SW_SHOWNORMAL);	
	}

	*pResult = 0;
}

void CRichEditCtrlGS::clear()
{
	SetWindowText(_T(""));
}
...
BOOL CRichEditCtrlGS::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
...
LRESULT CRichEditCtrlGS::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
