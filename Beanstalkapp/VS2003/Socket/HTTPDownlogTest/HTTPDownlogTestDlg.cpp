...
void CHTTPDownlogTestDlg::OnBnClickedButton3()
{
	CDownFileBase			downFile;

	downFile.DoFromNetPath("\\\\192.168.1.89\test\test.exe", "c:\\downlog");
}

void CHTTPDownlogTestDlg::NotifyUI(int nType, LPVOID lpParameter)
{
	if(CDownFileBase::NUIE_SetPorcess == nType)
	{
		m_progress.SetPos((int)lpParameter);
	}
}
DWORD CALLBACK _DownFiletest(LPVOID lpParameter)
{
	CHTTPDownlogTestDlg*	pThis		= (CHTTPDownlogTestDlg *)lpParameter;
	CString					strURL;

	pThis->GetDlgItemText(IDC_EDIT1, strURL);
	if(FALSE != pThis->DoFromHTMLServer(strURL, "C:\\MyDownlog"))
	{
		AfxMessageBox("下载成功");
	}
	else
	...
	//downFile.DoFromNetPath("\\\\192.168.1.89\\test\\test.exe", "c:\\MyDownLog", "Administrator", "123", TRUE);

	return 0;
}
void CHTTPDownlogTestDlg::OnBnClickedButton4()
{
	CreateThread(NULL, 0, _DownFiletest, this, 0, NULL);
}
