...
#define CHECK_PRIVILEGE_VOID(_ID_PRIV) \
	if(g_pCPrivilege->QueryPrivilege(_ID_PRIV) == FALSE)\
	{\
	return;\
	}\

CMyTabPage::CMyTabPage(void)
{
}
...
extern BOOL	g_bEnableCtl;
/*
void CMyTabPage::SetSelectedItem(CXTPTabManagerItem* pItem)
{
	if(g_bEnableCtl == FALSE)
		return;

	int		nIndex;
	nIndex = pItem->GetIndex();
	if(nIndex == 1 || nIndex == 2 || nIndex == 3 || nIndex == 4)
	{
		BOOL		bRetVal;
		bRetVal = CPassSet::CheckPassSet();
		if(bRetVal == FALSE)
			return;
	}
	...
	if(nIndex == 1 || nIndex == 2 || nIndex == 3 || nIndex == 4)
	{
		CHECK_PRIVILEGE_VOID(Privilege_Edit);
	}

	return __super::SetSelectedItem(pItem);
}
*/

void CMyTabPage::SetSelectedItem(int nId)
{
	if(g_bEnableCtl == FALSE)
		return;

	if(nId == 1 || nId == 2 || nId == 3 || nId == 4)
	{
		BOOL		bRetVal;
		bRetVal = CPassSet::CheckPassSet();
		if(bRetVal == FALSE)
	...
	return __super::SetSelectedItem(nId);
}
...
{
	m_nStyle = TCS_DOWN | TCS_MONOSPACE;
	m_nActivePage = -1;
	m_nBorder = 1;
	m_bEraseBkgnd = TRUE;
}
...
void CCoolTabCtrl::SetSelectedItem(int nId)
{
	SetActivePage(nId);
}

BOOL	CCoolTabCtrl::GetCurSel()
{
	return m_nActivePage;
}


BOOL	CCoolTabCtrl::SetCurSel(int nId)
{
	SetSelectedItem(nId);
	return TRUE;
}

CWnd *	CCoolTabCtrl::GetItem(int nId)
{
	CPageItem *		pItem;

	pItem = (CPageItem*)GetPageItem(nId);
	if(pItem == NULL) 
		return NULL;
...
CWnd *	CCoolTabCtrl::GetSelectedItem()
{
	return GetItem(GetCurSel());
}


BOOL	CCoolTabCtrl::InsertItem(CHAR *pText, CWnd *pWnd)
{
...
int		CCoolTabCtrl::GetItemCount()
{
	return m_PageList.GetCount();
}


BOOL	CCoolTabCtrl::SetCaption(int nId, CHAR *pText)
{
	CPageItem *		pItem;
